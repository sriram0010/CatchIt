...
class _ParticlesMotionState extends State<ParticlesMotion>
  ...
  update() {
    ...
    for (int i = 0; i < widget.totalParticles; i++) {
      ...
      particle.x += (cos(angle + particle.d) + 1 + particle.r / 2) * widget.speed;
      particle.y -= sin(angle) ; // Updated to subtract y value

      if (particle.x > width + 5 ||
          particle.x < -5 ||
          particle.y < 1) { // Updated to check if y value is less than -5
...
class ParticlePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < particles!.length; i++) {
      ...
      final position = Offset(particle.x, particle.y-10);
